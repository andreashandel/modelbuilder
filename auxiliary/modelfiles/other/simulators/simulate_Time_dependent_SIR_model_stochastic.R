...
#' result <- simulate_Time_dependent_SIR_model_stochastic() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_Time_dependent_SIR_model_stochastic(S = 2000,I = 2,R = 0) 
...
simulate_Time_dependent_SIR_model_stochastic <- function(S = 1000, I = 1, R = 0, b = 0.007, g = 0.09, d = 1, tfinal = 100, rngseed = 123) 
{ 
  #Block of ODE equations for adaptivetau 
  Time_dependent_SIR_model_fct <- function(y, parms, t) 
  {
    with(as.list(c(y,parms)),   
     { 
       #specify each rate/transition/reaction that can happen in the system 
     rates = c(b*(exp(-t/d))*S*I, g*I)
     return(rates) 
      }
	 	)   
  ...
  simout = adaptivetau::ssa.adaptivetau(init.values = varvec, transitions = transitions,
                  	 	 	 rateFunc = Time_dependent_SIR_model_fct, params = parvec, tf = tfinal) 
