...
#' result <- simulate_Time_dependent_SIR_model_discrete() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_Time_dependent_SIR_model_discrete(S = 2000,I = 2,R = 0) 
...
simulate_Time_dependent_SIR_model_discrete <- function(S = 1000, I = 1, R = 0, b = 0.007, g = 0.09, d = 1, tstart = 0, tfinal = 100, dt = 0.1) 
{ 
  #Function that encodes simulation loop 
  Time_dependent_SIR_model_fct <- function(vars, pars, times) 
  {
    with( as.list(c(vars,pars)), {  
      ...
      {
        ts[ct,] = c(t,S,I,R) 
        Sp = S + dt*(-b*(exp(-t/d))*S*I) 
        Ip = I + dt*(+b*(exp(-t/d))*S*I -g*I) 
        Rp = R + dt*(+g*I) 
        S = Sp 
        I = Ip 
        R = Rp 
  ...
  parvec = c(b = b, g = g, d = d) 
  timevec = seq(tstart, tfinal,by = dt) 
  #Running the model 
  simout <- Time_dependent_SIR_model_fct(vars = varvec, pars = parvec, times = timevec) 
