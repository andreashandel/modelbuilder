...
#' result <- simulate_Influenza_OAS_simpleModel_discrete() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_Influenza_OAS_simpleModel_discrete(Hf = 20000,Hb = 0,B1 = 200,A1 = 200,B2 = 20,A2 = 20) 
...
simulate_Influenza_OAS_simpleModel_discrete <- function(Hf = 10000, Hb = 0, B1 = 100, A1 = 100, B2 = 10, A2 = 10, k1 = 0.01, k2 = 0.02, df = 0.5, db = 0.7, s1 = 1, p1 = 1000, g1 = 0.1, d1 = 0.1, s2 = 0.5, p2 = 1000, g2 = 0.1, d2 = 0.1, tstart = 0, tfinal = 100, dt = 0.1) 
{ 
  #Function that encodes simulation loop 
  Influenza_OAS_simpleModel_fct <- function(vars, pars, times) 
  {
    with( as.list(c(vars,pars)), {  
      ...
      for (t in times) 
      {
        ...
        B1p = B1 + dt*(+s1*B1*Hf/(p1+Hf)) 
        A1p = A1 + dt*(+g1*B1 -k1*A1*Hf -d1*A1) 
        B2p = B2 + dt*(+s2*B2*Hf/(p2+Hf)) 
  ...
  #Running the model 
  simout <- Influenza_OAS_simpleModel_fct(vars = varvec, pars = parvec, times = timevec) 
